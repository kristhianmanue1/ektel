...
4. **Replay store durable y obligatorio, con dos registros distintos
   (ronda correctiva 2026-08-19, B3; detalle cerrado 2026-08-20, C4/D3):**
   - `nonce_reservation`: CAS durable **durante `admit`** — clave del
     registro: `(issuer_id, nonce)`; estados: `free → reserved`; el nonce
     se reserva antes de emitir la admisión (append con fsync de archivo y
   ...
   - `start_token_consumption`: CAS durable **inmediatamente antes de
     crear el proceso** — clave del registro: `identity_digest`; estados:
     `unspent → spent`. El `admitted_action` se gasta en `start`, no en
     `admit`. El perdedor de un `start` concurrente recibe
     `capability_rejected` con código cerrado de consumo ya registrado.
   **Recuperación (vocabulario cerrado, D3):** un crash después del CAS de
   consumo y antes del spawn deja el token **permanentemente gastado** y
   produce `start_failed_indeterminate` (estado de `StartOutcome`,
   ADR-005) — nunca «ausencia/fallo» ambiguo y nunca replay. La operación
   se reintenta como **nueva admisión con nonce nuevo**. La reconciliación
   consulta el store por `identity_digest` (mismo patrón que
   `AuditSink.query(event_id)`). El store **sobrevive reinicios** del
   runtime: tras reiniciar, ambos registros cargados siguen rechazando
