...
| A3 | Un `append` que devuelve `durable` (valor hoy renombrado `flush_protocol_completed`, D1) pero miente (fsync falló y el sink no lo notó) es indetectable. | **Refutada:** la honestidad del sink es responsabilidad del adaptador del operador; ektel declara la clase devuelta, no audita el disco del sink. Ya cubierto por "accepted_undemonstrated" para sinks que no pueden demostrar. |
